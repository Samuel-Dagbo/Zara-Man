...
import { HiOutlineShoppingBag, HiOutlineCheck, HiOutlineX, HiOutlineTruck } from 'react-icons/hi';
    ...
    <div>
        ...
        {['all', 'pending', 'confirmed', 'processing', 'shipped', 'delivered', 'cancelled'].map(status => (
          <button
            ...
            className={`px-4 py-2 text-xs tracking-wider uppercase transition-all duration-200 ${
              statusFilter === status ? 'bg-espresso text-cream' : 'bg-white border border-luxury-200 text-luxury-600 hover:border-espresso'
            }`}
          >
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="text-center py-12 text-luxury-500">No orders found.</div>
        ) : (
              ...
              <div className="mt-4 pt-4 border-t border-luxury-100 flex flex-wrap items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {order.status !== 'cancelled' && order.status !== 'delivered' && (
                    <button onClick={() => updateStatus(order._id, 'cancelled')} className="px-3 py-1.5 text-xs tracking-wider uppercase border border-red-200 text-red-500 hover:bg-red-50 transition-all">
                      Cancel
                    </button>
                  )}
                </div>
